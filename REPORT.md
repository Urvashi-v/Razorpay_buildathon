...
**The sealed test set has never been scored.** This report is where the test-set
numbers go, once, at the end — and until then it holds none. Writing provisional
numbers here, even clearly labelled ones, is how a placeholder becomes a claim
three commits later.

Datasets have been generated and the baseline ladder has been trained and
evaluated **on the validation split**; those measured results live in
[docs/ladder_results.md](docs/ladder_results.md), which is regenerated by
`rto-sentinel train` and never hand-edited. They are not test-set results and are
not reproduced here. No calibrated model exists yet either, so the calibration
and decision sections below remain unfilled by construction.
